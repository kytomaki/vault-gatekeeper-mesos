...
$(GATEKEEPER) : cmd/gatekeeper/main.go
	$(DOCKER) docker.dev.br.hmheng.io/hmheng-infra/gatekeeper-build:v1.0.3-9-g2ae2bdf go build -ldflags "-X main.BuildTime=`date -u '+%Y-%m-%d_%I:%M:%S%p'` -X main.Version=`git -C ./ describe --abbrev=0 --tags HEAD`" -a -installsuffix cgo -o src/github.com/nemosupremo/vault-gatekeeper/dist/gatekeeper github.com/nemosupremo/vault-gatekeeper/cmd/gatekeeper
